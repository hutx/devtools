package com.iquantex.devtools.base.server.impl;
...
public abstract class BaseServiceImpl<T> implements BaseService<T> {
    ...
    public List<T> insertSelectiveBacth(List<T> entitys) throws Exception {
        for (T entity : entitys) {
            if (getRealMapper().existsWithPrimaryKey(entity)) {
    ...
    public int bacthDelete(List<T> ids) throws Exception {
        int i = 0;
        for (T id : ids) {
