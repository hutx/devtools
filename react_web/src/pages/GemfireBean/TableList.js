      ...
      <Modal
        ...
        <Form layout="vertical">
          ...
          <FormItem
            ...
            label="类名称">
            {form.getFieldDecorator('className', {
              initialValue: formVals.className,
              rules: [
                {
                  required: true,
                  message: '请输入类型名称！'
          ...
          <FormItem
            ...
            label="类中文名称">
            {form.getFieldDecorator('classCnName', {
              initialValue: formVals.classCnName,
              rules: [
                {
                  required: true,
                  message: '请输入类中文名称！'
          ...
          <FormItem
            ...
            label="描述信息">
            {form.getFieldDecorator('remark', {initialValue: formVals.remark})(<Input placeholder="请输入描述信息"/>)}
          </FormItem>
...
@connect(({gemfire, loading}) => ({gemfire, loading: loading.models.gemfire}))
@Form.create()
class TableList extends PureComponent {
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    dispatch({type: 'gemfire/fetch', payload: params});
  };
  ...
  handleFormReset = () => {
    ...
    dispatch({type: 'gemfire/fetch', payload: {}});
  };
  ...
  handleDeleteClick = () => {
    ...
    // electedRows.forEach(row => ids.push({id:row.id})); selectedRows.map(row =>
    // row.id)
    dispatch({
      type: 'gemfire/remove',
      payload: selectedRows.map(row => ({id:row.id})),
      callback: () => {
  ...
  handleSearch = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      dispatch({type: 'gemfire/fetch', payload: values});
    });
  };
  ...
  handleAdd = fields => {
    ...
    dispatch({type: 'gemfire/add', payload: fields});
    this.handleModalVisible();
  };

  handleUpdate = fields => {
    const {dispatch} = this.props;
    dispatch({type: 'gemfire/update', payload: fields});
    dispatch({type: 'gemfire/fetch'});
  ...
  render() {
    ...
    return (
      <PageHeaderWrapper title="GemfireBean">
        ...
        <CreateForm {...parentMethods} modalVisible={modalVisible}/> {stepFormValues && Object
          .keys(stepFormValues)
          .length
          ? (<UpdateForm
            {...updateMethods}
        ...
        {stepFormValues && Object
          ...
          ? (<EditForm
            {...editBeanMethods}
            modalVisible={editBeanModalVisible}
            values={stepFormValues} />)
          : null}
      </PageHeaderWrapper>
    );
